...
static struct rxq ports[MAX_PORTS];
...
sys_bind(void) {
  ...
  if (ports[port].bound) return -1;  
  
  ports[port].bound = 1;
  ports[port].head = ports[port].tail = ports[port].count = 0;
  initlock(&ports[port].lock, "udp_rxq");
  return 0;
}
...
sys_recv(void) {
  ...
  if (dport < 0 || dport >= MAX_PORTS || !ports[dport].bound)
      return -1;
  ...
  char *buf = ports[dport].packets[ports[dport].head];
  ports[dport].head = (ports[dport].head + 1) % RXQ_SIZE;
  ports[dport].count--;
  release(&ports[dport].lock);

  struct eth *eth = (struct eth *)buf;
  struct ip *iph = (struct ip *)(eth + 1);
  struct udp *udph = (struct udp *)(iph + 1);

  uint32 src_ip = ntohl(iph->ip_src);
  uint16 src_port = ntohs(udph->sport);
  char *payload = (char *)(udph + 1);
  int payload_len = ntohs(udph->ulen) - 8;  
  ...
  if (copyout(p->pagetable, sport_addr, (char *)&src_port, sizeof(src_port)) < 0)
      goto err;
  if (copyout(p->pagetable, buf_addr, payload, payload_len) < 0)
      goto err;
...
ip_rx(char *buf, int len)
{
  ...
  ports[dport].packets[ports[dport].tail] = buf;
  ports[dport].tail = (ports[dport].tail + 1) % RXQ_SIZE;
  ports[dport].count++;
  release(&ports[dport].lock);
